...
function BreakdownList({
    ...
    if (items.length === 0) {
        return (
            <div className="text-meta text-white/80">No {title.toLowerCase()} spend in this range.</div>
        );
    }
    ...
    return (
        <div className="space-y-2 min-w-[240px]">
            <div className="flex items-center justify-between gap-3 pb-1.5 border-b border-white/15">
                <span className="flex items-center gap-1.5 text-meta font-bold uppercase tracking-wider">
                    <span className={`w-1.5 h-1.5 rounded-full ${dot}`} />
                    <span className={accent}>{title}</span>
                </span>
                <span className="text-meta font-bold tabular-nums text-white">{formatCurrency(total)}</span>
            </div>

            <div>
                <p className="text-micro uppercase tracking-widest font-bold text-white/60 mb-1">Top categories</p>
                <ul className="space-y-0.5">
                    {categories.slice(0, 3).map(c => (
                        <li key={c.category} className="flex justify-between text-meta">
            ...
            <div>
                <p className="text-micro uppercase tracking-widest font-bold text-white/60 mb-1">Top items</p>
                <ul className="space-y-0.5">
                    {items.slice(0, 4).map(i => (
                        <li key={i.label} className="flex justify-between text-meta">
        ...
        <TooltipProvider delayDuration={150}>
            <Card className="bg-card/40 backdrop-blur-md border-white/5 shadow-none">
                <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between gap-2">
                        <h3 className="font-bold text-body uppercase tracking-wider text-muted-foreground/80 flex items-center gap-1.5">
                            ...
                            <Tooltip>
                                ...
                                <TooltipContent side="bottom" align="start" className={`${TOOLTIP_SURFACE} max-w-[300px] p-3 text-meta leading-relaxed`}>
                        ...
                        <span
                            className="text-caption font-bold tabular-nums px-2 py-0.5 rounded-md bg-secondary/30 text-muted-foreground"
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <button type="button" className="text-left rounded-xl bg-violet-500/10 border border-violet-500/20 px-3 py-2 cursor-help hover:bg-violet-500/15 transition-colors">
                                    <p className="text-micro uppercase tracking-widest font-bold text-violet-300/80">Recurring</p>
                                    <p className="text-body font-bold mt-0.5 tabular-nums text-violet-200">
                                        {formatCurrency(recurringTotal)}
                                        <span className="ml-1.5 text-caption text-violet-300/60">{recurringPct.toFixed(0)}%</span>
                                    </p>
                                </button>
                            </TooltipTrigger>
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <button type="button" className="text-left rounded-xl bg-cyan-500/10 border border-cyan-500/20 px-3 py-2 cursor-help hover:bg-cyan-500/15 transition-colors">
                                    <p className="text-micro uppercase tracking-widest font-bold text-cyan-300/80">Discretionary</p>
                                    <p className="text-body font-bold mt-0.5 tabular-nums text-cyan-200">
                                        {formatCurrency(discretionaryTotal)}
                                        <span className="ml-1.5 text-caption text-cyan-300/60">{discretionaryPct.toFixed(0)}%</span>
                                    </p>
                                </button>
                            </TooltipTrigger>
                    ...
                    {recurringTotal === 0 && (
                        <p className="text-caption text-muted-foreground/70 text-center pt-1">
