    ...
    (
        ...
        return (
            <Input
                ...
                name={name || picker}
                className={cn(
                    "w-[42px] h-8 text-center font-mono text-sm tabular-nums caret-transparent focus:bg-accent focus:text-accent-foreground [&::-webkit-inner-spin-button]:appearance-none",
                    className
                )}
    ...
    ({ period, setPeriod, date, setDate, onLeftFocus, onRightFocus }, ref) => {
        ...
        return (
            <Select value={period} onValueChange={(value: Period) => handleValueChange(value)}>
                <SelectTrigger ref={ref} size="sm" className="w-[65px] h-8 text-sm focus:bg-accent focus:text-accent-foreground py-1" onKeyDown={handleKeyDown}>
                    <SelectValue />
                </SelectTrigger>
                <SelectContent>
